...
microsynth <- function(data, idvar, intvar, timevar = NULL, start.pre = NULL,
    end.pre = NULL, end.post = NULL, match.out = TRUE, match.covar = TRUE, match.out.min = NULL,
    match.covar.min = NULL, result.var = TRUE, omnibus.var = result.var, period = 1,
    scale.var = "Intercept", confidence = 0.9, test = "twosided", perm = 0, jack = 0,
    use.survey = TRUE, cut.mse = Inf, check.feas = FALSE, use.backup = FALSE,
    w = NULL, max.mse = 0.01, maxit = 250, cal.epsilon = 1e-04, calfun = "linear",
    ...
    # Set up dummy variables for use in determining values of indirectly
    # stated inputs
    ...
    if (length(match.covar) == 0) {
        ...
        if (!reset.match.covar) {
            ...
            if (printFlag) {
                message("match.covar = c(\"", paste(match.covar, collapse = "\",\"",
                  sep = ""), "\")\n\n", sep = "", appendLF = FALSE)
            }
        }
    }
    ...
    if (!is.list(match.out.min) & length(match.out.min) > 0) {
        ...
        match.out.min <- check.matchout(match.out.min, end.pre - min(times) +
            1)
        if (match.out.min[[2]]) {
            if (printFlag) {
    ...
    if (is.logical(omnibus.var)) {
        if (omnibus.var) {
            if (length(match.out) == 1) {
                ...
                if (printFlag) {
                  message("omnibus.var = c(\"", paste(omnibus.var, collapse = "\",\"",
                    sep = ""), "\")\n\n", sep = "", appendLF = FALSE)
                }
            }
        } else {
    ...
    if (length(w) == 0) {
        ...
        w <- get.w(data, match.covar, match.covar.min, match.out, match.out.min,
            boot = perm, jack = jack, Int = Intervention[, as.character(end.pre)],
            int.val = int.num, trim = NULL, end.pre = end.pre, cal.epsilon = cal.epsilon,
            maxit = maxit, bounds = bounds, calfun = calfun, check.feas = check.feas,
            scale.var = scale.var, cut.mse = max.mse, use.backup = use.backup,
            time.names = time.names, printFlag = printFlag, n.cores = n.cores)
        tmp <- proc.time() - tmp
        if (printFlag) {
            message("Calculation of weights complete: Total time = ", round(tmp[3],
                2), "\n\n", sep = "", appendLF = FALSE)
        }
    } else {
        if (printFlag) {
    ...
    if (!reset.result.var) {
        ...
        for (i in 1:length(end.post)) {
            ...
            stats[[i]] <- get.stats(data, w$Weights, w$Intervention, w$keep.groups,
                result.var, end.pre = end.pre, period = period, end.post = end.post[i],
                omnibus.var = omnibus.var, start.pre = start.pre, cut.mse = cut.mse,
            ...
            if (!reset.result.var) {
                ...
                stats.tmp <- get.stats1(data, w.tmp, Inter.tmp, mse.tmp, result.var,
                  end.pre = end.pre, period = period, end.post = end.post[i],
                  omnibus.var = omnibus.var, twosided = twosided, printFlag = printFlag,
                  n.cores = n.cores)
                ...
                if (jack > 0) {
                  ...
                  Jack.CI <- make.ci(jack.stats2, sqrt(jack.delta.out), alpha = 1 -
                    confidence)
                ...
                if (perm > 0 & use.survey) {
                  ...
                  Perm.CI <- make.ci2(synth.stats2, synth.delta.out, alpha = 1 -
                    confidence)
                ...
                results[[i]] <- cbind(Trt, Con, Pct.Chng, Linear.pVal, Linear.CI,
                  Jack.pVal, Jack.CI, Perm.pVal, Perm.CI)
                if (NROW(results[[i]]) == 1) {
...
# Sub-function of microsynth(); reshape data to make 3D array with other
# necessary outputs
newreshape <- function(data, timevar, idvar, intvar, v.names = NULL, nv.names = NULL) {
    ...
    if (length(times) > 1) {
        ...
        colnames(newdat)[append.cols] <- paste(colnames(newdat)[append.cols],
            ".", times, sep = "")
    }
    intcols <- substr(colnames(newdat), 1, nchar(intvar)) == intvar
    intcols <- intcols & (substr(colnames(newdat), nchar(intvar) + 1, nchar(intvar) +
        1) == "" | substr(colnames(newdat), nchar(intvar) + 1, nchar(intvar) +
        1) == ".")
    ...
    for (i in 1:dim(out)[2]) {
        ...
        here <- here & (substr(colnames(newdat), nchar(nam.tmp) + 1, nchar(nam.tmp) +
            1) == "." | substr(colnames(newdat), nchar(nam.tmp) + 1, nchar(nam.tmp) +
            1) == "")
        tmp <- newdat[, here, drop = FALSE]
        if (sum(here, na.rm = TRUE) > 1) {
            colnames(tmp) <- gsub(paste(dimnames(out)[[2]][i], ".", sep = ""),
                "", colnames(tmp))
...
out.results <- function(results, end.pre, end.post = names(results), file = NULL,
    ...
    if (use.xlsx) {
        ...
        for (i in 1:length(end.post)) {
            ...
            xlsx::addDataFrame(out, sheet, colStyle = c(pct.list, pval.list,
                oth.list))

            bord1 <- xlsx::Border(color = "black", position = "BOTTOM", pen = "BORDER_THIN")
            cb <- xlsx::CellBlock(sheet, startRow = 1, startColumn = 1, noRows = (n +
                1), noColumns = (p + 1), create = FALSE)
            xlsx::CB.setBorder(cb, border = bord1, rowIndex = 1, colIndex = 1:(p +
                1))

            font <- xlsx::Font(wb, isBold = TRUE)
            xlsx::CB.setFont(cb, font = font, rowIndex = 1, colIndex = 2:(p +
                1))
...
make.ci2 <- function(stats, delta.out, alpha = 0.05) {
    ...
    quants <- apply(z.scores, 2, stats::quantile, probs = c(alpha/2, 1 - alpha/2),
        na.rm = TRUE)
...
remove.vars <- function(vars, nams, objnam = "result.var", printFlag = TRUE) {
    ...
    if (length(rm.vars) > 0) {
        ...
        vars <- vars[-rm.here]
        if (printFlag) {
            message("WARNING: The following variables will be removed from ",
                objnam, " since they are not in the dataset: \n", sep = "", appendLF = FALSE)
        }
        if (printFlag) {
