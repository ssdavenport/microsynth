...
get.w <- function(bigdat, covar.var, covar.var1 = NULL, dum, dum1 = NULL, boot = 0, jack = 0, Int, int.val = 1, trim = NULL, maxit = 500, cal.epsilon = 1e-04, end.pre, bounds = c(-Inf, Inf), calfun = "raking", qpmeth = "LowRankQP", check.feas = FALSE, use.backup = TRUE, scale.var = "Intercept", cut.mse = 1, time.names = NULL,
    ...
    for (i in 1:for.max) {
        ...
        if (use.model.i == 1) {
            mod[i] <- "First"
            ws <- get.w.sub(newdat = newdat, newdat1 = newdat1, end.pre = end.pre, samp = samp, use = use, n = NROW(newdat), maxit = maxit, calfun = calfun, bounds = bounds, epsilon = cal.epsilon, trim = trim, qpmeth = qpmeth, scale.var = scale.var, printFlag = printFlag)
        ...
        if (use.model.i == 2 & use.backup) {
            mod[i] <- "Second"
            ws <- get.w.sub(newdat = newdata, newdat1 = newdat1a, end.pre = end.pre, samp = samp, use = use, n = NROW(newdat), maxit = maxit, calfun = calfun, bounds = bounds, epsilon = cal.epsilon, trim = trim, qpmeth = qpmeth, scale.var = scale.var, printFlag = printFlag)
            if (ws$mse > cut.mse | is.na(ws$mse)) {
                if (i > 1) {
        ...
        if (use.model.i == 3 & use.backup) {
            mod[i] <- "Third"
            ws <- get.w.sub(newdat = newdatb, newdat1 = newdat1b, end.pre = end.pre, samp = samp, use = use, n = NROW(newdat), maxit = maxit, calfun = calfun, bounds = bounds, epsilon = cal.epsilon, trim = trim, qpmeth = qpmeth, scale.var = scale.var, printFlag = printFlag)
        }
    ...
    if (boot + jack + 1 > for.max) {
        ...
        list.out <- parallel::parLapply(cl = cl, X = (for.max + 1):(boot + jack + 1), get.w.sub.par, use.model, Int, int.val, colnam, rep.G, n, fin.boots, boots, newdat, newdat1, newdata, newdat1a, newdatb, newdat1b, end.pre, maxit, calfun, bounds, cal.epsilon, trim, qpmeth, scale.var, printFlag = FALSE, cut.mse, use.backup,
            jack, back.state, back.state1, back.state2, back.state3, tmp.jack, boot, boot.lower, boot.upper, jack.lower, jack.upper, rownams = rownames(wghts), rownams1 = rownames(mse))
...
get.w.sub <- function(newdat = NULL, newdat1 = NULL, bigdat = NULL, dum = NULL, dum1 = NULL, covar.var = NULL, covar.var1 = NULL, end.pre, samp, use, n = NROW(newdat), maxit = 500, calfun = "raking", bounds = c(-Inf, Inf), epsilon = 1e-04, trim = NULL, qpmeth = "LowRankQP", scale.var = "Intercept", printFlag = TRUE) {
    tmp <- proc.time()
...
get.w.sub.par <- function(X, use.model, Int, int.val, colnam, rep.G, n, fin.boots, boots, newdat, newdat1, newdata, newdat1a, newdatb, newdat1b, end.pre, maxit, calfun, bounds, cal.epsilon, trim, qpmeth, scale.var, printFlag, cut.mse, use.backup, jack, back.state, back.state1, back.state2, back.state3, tmp.jack, boot,
    boot.lower, boot.upper, jack.lower, jack.upper, rownams, rownams1) {
...
get.mse <- function(newdat, newdat1 = NULL, samp, use, ws, ws.init, scale.by) {
    ...
    if (length(newdat1) == 0) {
        ...
        # printstuff <- cbind(Targets = c(targets, targets1), Initial.Weighted.Control = colSums(ws.init * cbind(condat, condat1)), Final.Weighted.Control = colSums(ws * cbind(condat, condat1)), All.scaled = scale.by * colSums(cbind(alldat, alldat1)))
...
my.qp <- function(b.init, X, Y, a, c, M = 10000, qpmeth = "LowRankQP", maxit = 1000, printFlag = TRUE) {
    ...
    # if (qpmeth == 'nleqslv') { requireNamespace('nleqslv', quietly = TRUE) XtX <- crossprod(X) M <- 1/M jacobian <- function(all) { b <- all[1:q] lambda <- all[(q + 1):(q + n)] lowerright <- matrix(0, n, n) upperleft <- diag(1/b) + XtX upperright <- t(Y) lowerleft <- Y out <- rbind(cbind(upperleft, upperright),
    # cbind(lowerleft, lowerright)) return((out)) } f1 <- function(all) { b <- all[1:q] lambda <- all[(q + 1):(q + n)] out1 <- (M * log(b)) + crossprod(X, (X %*% b - a)) - crossprod(Y, lambda) out2 <- Y %*% b - c return(c(out1, out2)) } b.init[b.init < 1e-06] <- 1e-06 lambda.init <- solve(tcrossprod(Y), Y) %*% ((M *
    # log(b.init)) + crossprod(X, (X %*% b.init - a))) all.init <- c(b.init, lambda.init) all.root <- nleqslv::nleqslv(all.init, fn = f1, jac = NULL, method = c('Broyden'), global = c('gline'), xscalm = c('fixed'), control = list(maxit = maxit, xtol = 1e-11, ftol = 1e-11, btol = 1e-06, cndtol = 1e-13)) if (printFlag)
    # { message('Number of Jacobian evaluations = ', all.root$njcnt, '. \n', sep = '', appendLF = FALSE) } if (printFlag) { message('Number of function evaluations = ', all.root$nfcnt, '. \n', sep = '', appendLF = FALSE) } if (printFlag) { message('Number of iterations = ', all.root$iter, '. \n', sep = '', appendLF =
    # FALSE) } all.root <- all.root$x b <- all.root[1:q] b[b < 0] <- 0 } else
    if (qpmeth == "LowRankQP") {
        # requireNamespace('LowRankQP', quietly = TRUE)
