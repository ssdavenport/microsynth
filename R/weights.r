...
get.w <- function(bigdat, covar.var, covar.var1 = NULL, dum, dum1 = NULL, boot = 0,
    jack = 0, Int, int.val = 1, trim = NULL, maxit = 500, cal.epsilon = 1e-04,
    end.pre, bounds = c(-Inf, Inf), calfun = "raking", qpmeth = "LowRankQP",
    check.feas = FALSE, use.backup = TRUE, scale.var = "Intercept", cut.mse = 1,
    time.names = NULL, keep.int = FALSE, printFlag = TRUE, n.cores = 1) {
    ...
    if (boot > 0) {
        ...
        if (n.choose - 1 <= max(1e+06, boot)) {
            ...
            is.trt.area <- which(apply(boots, 2, check.combn, x = which(Int ==
                int.val)) == 0)
            boots <- boots[, base::sample((1:NCOL(boots))[-is.trt.area], boot),
                drop = FALSE]
            fin.boots <- TRUE
        }
    }
    ...
    if (check.feas & use.backup) {
        ...
        is.sol <- is.feasible(bigdat, covar.var, dum, Int = Int, int.val = int.val,
        ...
        if (!is.sol) {
            ...
            is.sol <- is.feasible(bigdat, covar.var, dum.tmp[[1]], Int = Int,
                int.val = int.val, end.pre = end.pre, eps = 1e-04)
            tmp <- proc.time() - tmp
            if (!is.sol) {
                use.model <- 3
                if (printFlag) {
                  message("Second model is infeasible: Time = ", round(tmp[3],
                    2), "\n", sep = "", appendLF = FALSE)
                }
                if (printFlag) {
                ...
                if (printFlag) {
                  message("Second model is feasible: Time = ", round(tmp[3],
                    2), "\n", sep = "", appendLF = FALSE)
                }
            }
        } else {
    ...
    for (i in 1:for.max) {
        ...
        if (use.model.i == 1) {
            mod[i] <- "First"
            ws <- get.w.sub(newdat = newdat, newdat1 = newdat1, end.pre = end.pre,
                samp = samp, use = use, n = NROW(newdat), maxit = maxit, calfun = calfun,
                bounds = bounds, epsilon = cal.epsilon, trim = trim, qpmeth = qpmeth,
            ...
            if (ws$mse > cut.mse | is.na(ws$mse)) {
                ...
                if (i > 1) {
                  back.state1 <- paste("First model was infeasible for ", rep.meth,
                    " group ", g, cat.back1, "\n", sep = "")
                } else {
        ...
        if (use.model.i == 2 & use.backup) {
            mod[i] <- "Second"
            ws <- get.w.sub(newdat = newdata, newdat1 = newdat1a, end.pre = end.pre,
                samp = samp, use = use, n = NROW(newdat), maxit = maxit, calfun = calfun,
                bounds = bounds, epsilon = cal.epsilon, trim = trim, qpmeth = qpmeth,
                scale.var = scale.var, printFlag = printFlag)
            if (ws$mse > cut.mse | is.na(ws$mse)) {
                if (i > 1) {
                  back.state2 <- paste("Second model was infeasible for ", rep.meth,
        ...
        if (use.model.i == 3 & use.backup) {
            mod[i] <- "Third"
            ws <- get.w.sub(newdat = newdatb, newdat1 = newdat1b, end.pre = end.pre,
                samp = samp, use = use, n = NROW(newdat), maxit = maxit, calfun = calfun,
                bounds = bounds, epsilon = cal.epsilon, trim = trim, qpmeth = qpmeth,
                scale.var = scale.var, printFlag = printFlag)
        }
        ...
        mses <- get.mse(newdat, newdat1, samp, use, ws$wghts, ws$wghts.init,
            ws$scale.by)
        msesa <- get.mse(newdata, newdat1a, samp, use, ws$wghts, ws$wghts.init,
            ws$scale.by)
        msesb <- get.mse(newdatb, newdat1b, samp, use, ws$wghts, ws$wghts.init,
            ws$scale.by)
        mse[1, i] <- mses$mse
        mse[2, i] <- mses$mse1
        mse[3, i] <- msesa$mse
        ...
        if (i == 1) {
            ...
            if (use.model.i == 1) {
                ...
                if (printFlag) {
                  message(paste0(utils::capture.output(round(printstuff, 4)),
                    collapse = "\n"), appendLF = FALSE)
                }
                if (printFlag) {
                  message("\n\n", appendLF = FALSE)
    ...
    if (boot + jack + 1 > for.max) {
        ...
        list.out <- parallel::parLapply(cl = cl, X = (for.max + 1):(boot + jack +
            1), get.w.sub.par, use.model, Int, int.val, colnam, rep.G, n, fin.boots,
            boots, newdat, newdat1, newdata, newdat1a, newdatb, newdat1b, end.pre,
            maxit, calfun, bounds, cal.epsilon, trim, qpmeth, scale.var, printFlag = FALSE,
            cut.mse, use.backup, jack, back.state, back.state1, back.state2,
            back.state3, tmp.jack, boot, boot.lower, boot.upper, jack.lower,
            jack.upper, rownams = rownames(wghts), rownams1 = rownames(mse))
        ...
        if (printFlag) {
            message("Completed calculation of all replication weights: Time = ",
                round(tmp.boot[3], 2), "\n\n", sep = "", appendLF = FALSE)
        }
    ...
    keep.groups <- mse[2 * (use.model - 1) + 1, ] < cut.mse & !is.na(mse[2 *
        (use.model - 1) + 1, ])
    ...
    out <- list(Weights = wghts, Intervention = Inter, MSE = mse, Model = mod,
        Summary = printstuff, keep.groups = keep.groups, num.constr = c(num.exact = num.exact,
            num.prox = num.prox))
    return(out)
}
    ...
    if (length(nam1) > 0) {
        ...
        for (i in 1:length(nam)) {
            ...
            if (length(here1) == 0 & length(here2) == 0) {
                ...
                if (length(dumc) > 1) {
                  dumc <- c(dumc[1], dumc[2:length(dumc)] - dumc[1:(length(dumc) -
                    1)])
                }
...
# Sub-function of get.w(); check if a constraint model has a feasible
# solution
...
# Sub-function of is.feasible(); check if a constraint model has a feasible
# solution
check.feasible2 <- function(A, b, eps = 1e-07, M = 10000, meth = "LowRankQP") {
    # Sub-function for checking to see if a constraint model has a feasible
    # solution
    if (NROW(A) <= NCOL(A)) {
    ...
    if (meth == "LowRankQP") {
        ...
        sup.out <- utils::capture.output(all.root <- LowRankQP::LowRankQP(Vmat = Vmat,
            dvec = a, Amat = A3, bvec = b, uvec = uvec, method = "SMW", verbose = FALSE,
            niter = maxit))
        sol <- all.root$alpha
    } else if (meth == "ipop") {
        requireNamespace("kernlab", quietly = TRUE)

        sup.out <- utils::capture.output(all.root <- kernlab::ipop(H = Vmat,
            c = a, A = A3, b = b, u = uvec, l = rep(0, length(a)), r = 0, verb = 0,
            maxiter = maxit))
...
# Sub-function of is.feasible(); reshape data inaccordance with a stated
# model structure Produces two data frames: newdat (data used for exact
# constraints) and newdat1 (data used for proximate constraints)
get.newdat <- function(bigdat, dum = NULL, dum1 = NULL, covar.var = NULL, covar.var1 = NULL,
    ...
    if (length(result.var) > 0) {
        for (j in 1:length(result.var)) {
            ...
            for (i in 1:length(lows)) {
                newdat[[j]][, i] <- rowSums(as.matrix(bigdat[, result.var[j],
                  lows[i]:highs[i]]))
                if (lows[i] == highs[i]) {
    ...
    if (length(dum1) > 0) {
        ...
        for (j in 1:length(result.var1)) {
            ...
            for (i in 1:length(lows)) {
                newdat1[[j]][, i] <- rowSums(as.matrix(bigdat[, result.var1[j],
                  lows[i]:highs[i]]))
                if (lows[i] == highs[i]) {
...
get.w.sub <- function(newdat = NULL, newdat1 = NULL, bigdat = NULL, dum = NULL,
    dum1 = NULL, covar.var = NULL, covar.var1 = NULL, end.pre, samp, use, n = NROW(newdat),
    maxit = 500, calfun = "raking", bounds = c(-Inf, Inf), epsilon = 1e-04, trim = NULL,
    qpmeth = "LowRankQP", scale.var = "Intercept", printFlag = TRUE) {
    tmp <- proc.time()
    ...
    form <- paste("~", paste(usevars[keep], collapse = "+", sep = ""), "-1",
        sep = "")
    ...
    if (NCOL(newdat) > 1) {
        tryCatch({
            ...
            cali2 <- survey::calibrate(design = caldesign, maxit = maxit, formula = form,
                population = targets[keep], data = condat[, keep], calfun = calfun,
    ...
    if (length(newdat1) > 0) {
        ...
        ws <- my.qp(b.init = ws.init, X = t(condat2), a = targets2, Y = t(condat[,
            keep, drop = FALSE]), c = targets[keep], qpmeth = qpmeth, printFlag = printFlag)
        tmp2 <- proc.time() - tmp2
    }
    ...
    if (length(trim) > 0) {
        ...
        cali2 <- survey::trimWeights(cali2, upper = stats::quantile(ws, max(trim)),
            lower = stats::quantile(ws, min(trim)))
...
get.w.sub.par <- function(X, use.model, Int, int.val, colnam, rep.G, n, fin.boots,
    boots, newdat, newdat1, newdata, newdat1a, newdatb, newdat1b, end.pre, maxit,
    calfun, bounds, cal.epsilon, trim, qpmeth, scale.var, printFlag, cut.mse,
    use.backup, jack, back.state, back.state1, back.state2, back.state3, tmp.jack,
...
get.mse <- function(newdat, newdat1 = NULL, samp, use, ws, ws.init, scale.by) {
    ...
    if (length(newdat1) == 0) {
        printstuff <- cbind(Targets = targets, Weighted.Control = colSums((ws) *
            (condat)), All.scaled = (scale.by) * colSums(alldat))
    } else {

        # printstuff <- cbind(Targets = c(targets, targets1),
        # Initial.Weighted.Control = colSums(ws.init * cbind(condat,
        # condat1)), Final.Weighted.Control = colSums(ws * cbind(condat,
        # condat1)), All.scaled = scale.by * colSums(cbind(alldat,
        # alldat1)))
        printstuff <- cbind(Targets = c(targets, targets1), Final.Weighted.Control = colSums(ws *
            cbind(condat, condat1)), All.scaled = scale.by * colSums(cbind(alldat,
            alldat1)))
    }
...
# Function used to solve quadratic optimization when proximate constraints
# are invoked
my.qp <- function(b.init, X, Y, a, c, M = 10000, qpmeth = "LowRankQP", maxit = 1000,
    ...
    # if (qpmeth == 'nleqslv') { requireNamespace('nleqslv', quietly =
    # TRUE) XtX <- crossprod(X) M <- 1/M jacobian <- function(all) { b <-
    # all[1:q] lambda <- all[(q + 1):(q + n)] lowerright <- matrix(0, n, n)
    # upperleft <- diag(1/b) + XtX upperright <- t(Y) lowerleft <- Y out <-
    # rbind(cbind(upperleft, upperright), cbind(lowerleft, lowerright))
    # return((out)) } f1 <- function(all) { b <- all[1:q] lambda <- all[(q
    # + 1):(q + n)] out1 <- (M * log(b)) + crossprod(X, (X %*% b - a)) -
    # crossprod(Y, lambda) out2 <- Y %*% b - c return(c(out1, out2)) }
    # b.init[b.init < 1e-06] <- 1e-06 lambda.init <- solve(tcrossprod(Y),
    # Y) %*% ((M * log(b.init)) + crossprod(X, (X %*% b.init - a)))
    # all.init <- c(b.init, lambda.init) all.root <-
    # nleqslv::nleqslv(all.init, fn = f1, jac = NULL, method =
    # c('Broyden'), global = c('gline'), xscalm = c('fixed'), control =
    # list(maxit = maxit, xtol = 1e-11, ftol = 1e-11, btol = 1e-06, cndtol
    # = 1e-13)) if (printFlag) { message('Number of Jacobian evaluations =
    # ', all.root$njcnt, '. \n', sep = '', appendLF = FALSE) } if
    # (printFlag) { message('Number of function evaluations = ',
    # all.root$nfcnt, '. \n', sep = '', appendLF = FALSE) } if (printFlag)
    # { message('Number of iterations = ', all.root$iter, '. \n', sep = '',
    # appendLF = FALSE) } all.root <- all.root$x b <- all.root[1:q] b[b <
    # 0] <- 0 } else
    if (qpmeth == "LowRankQP") {
        # requireNamespace('LowRankQP', quietly = TRUE)

        sup.out <- utils::capture.output(all.root <- LowRankQP::LowRankQP(Vmat = t(X),
            dvec = -crossprod(X, a), Amat = Y[leave, , drop = FALSE], bvec = c[leave],
            uvec = rep(M, q), method = "SMW", verbose = FALSE, niter = maxit))
        b <- all.root$alpha
    } else if (qpmeth == "ipop") {
        requireNamespace("kernlab", quietly = TRUE)

        sup.out <- utils::capture.output(all.root <- kernlab::ipop(H = t(X),
            c = -crossprod(X, a), A = Y[leave, , drop = FALSE], b = c[leave],
            u = rep(M, q), verb = 0, maxiter = maxit))
        # all.root <- kernlab::ipop(c = -crossprod(X, a), H = crossprod(X),
        # b = c, A = Y, l = rep(0, q), r = rep(0, n), u = rep(M, q))
