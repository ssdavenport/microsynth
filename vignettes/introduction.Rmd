...
* creating a plot and displaying it as output (`plot()`); to save to file, 
specify a .csv or .xlsx as the `file` argument in `plot()`. 
...
arguments are declared in the  call to `plot()`, the plots will include one row 
...
#TODO: get this to run; current it fails: "object 'plot.stats' not found"
...
```{r, eval = TRUE, echo = FALSE}
saveReducedMicrosynth(sea5, "../inst/extdata/sea5.rds")
sea5 <- readRDS("../inst/extdata/sea5.rds")
summary(sea5)
```
