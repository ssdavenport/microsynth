...
estimated; `omnibus.vars` will include them in the omnibus statistic; and 
...
specified * calculating a variance estimate based on the linearization method
only, and based on that, running a one-sided lower significance test
(`test=lower`) * calculating an omnibus statistic to test joint significance
across all outcome variables (`result.var = match.out`) * creating a plot and
displaying it as output (`plot.file=NULL`); to save to file, specify a .csv or
.xlsx. * estimating results and saving it to file.
...
```{r ex1, eval = TRUE, fig.show = "hold", fig.width = 6, fig.height = 7} 
...
After the call to microsynth has been made, the \code{microsynth.tab} function
...
```{r ex2, eval = TRUE, fig.show = "hold", fig.width = 6, fig.height = 7} 
sea2 <- microsynth(seattledmi, 
                   ...
                   test="lower", 
                   perm=2, jack=TRUE, 
                   plot.file=NULL, sep = TRUE, result.file="ExResults2.xlsx")
```

Calling \code{microsynth.tab} identifies other new changes to the results.
...
```{r ex3, eval = FALSE, fig.show = "hold", fig.width = 6, fig.height = 7} 
...
```{r ex3results, eval = FALSE}
microsynth.tab(sea3)$Results
```
...
```{r ex4, eval = FALSE, fig.show = "hold", fig.width = 6, fig.height = 7} 
...
```{r ex4results, eval = FALSE}
microsynth.tab(sea4)$Results
```
...
```{r ex6, eval = FALSE, fig.show = "hold", fig.width = 6, fig.height = 7}
...
weights only) can be saved to file or viewed using `microsynth.tab`.

```{r ex9results, eval = FALSE, fig.show = "hold", fig.width = 6, fig.height = 7}
