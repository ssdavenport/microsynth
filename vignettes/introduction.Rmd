...
```{r}
plot(sea2, plot.var=match.out, start.pre=1, end.pre=12, end.post=c(14, 16))
```
...
```{r ex4plot}
plot(sea4, plot.var = names(match.out))
```
...
TODO: update this section (ex6) for changes in code.
...
plot(sea5, plot.var=match.out[1:2])
```
...
plot(sea8, plot.var=match.out[4])
summary(sea8m)
```
